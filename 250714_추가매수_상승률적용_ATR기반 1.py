...
def adjust_buy_drop_based_on_atr(atr_percent):
    ...
    max_scale = 0.85  # 변동성 높을 때 (추가 매수 천천히)
...
def buy_coin(ticker):
    ...
    time.sleep(120) # 매수 후 잠시 대기시간
        ...
        # 매수 체결가 가져오기 (최신 체결 가격 사용)
    current_price = pyupbit.get_current_price(ticker)
    last_buy_prices[ticker] = current_price  # 마지막 매수 가격 업데이트
    return order
...
def sell_coin(ticker):
    ...
    if balance > 0:
        ...
        last_buy_prices[ticker] = None  # 매도 시 마지막 매수 가격 초기화
        return order
    return None
...
while True:
    try:
        for ticker in TICKERS:  
            ...
            # last_buy_price가 없으면 avg_buy_price 사용 (초기 매수 후)
            ...
            profit_rate = ((current_price - avg_buy_price) / avg_buy_price * 100) if avg_buy_price else 0
